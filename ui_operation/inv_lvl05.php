<?php
	...
	if( !isset( $_GET['ids'] ) ){
		...
		$subdivision_id = (int) test_inputs( $_GET['ids'] );
	}
...
<?php
	...
	$qu_inv_05_categories_sel = "SELECT * FROM  `inv_05_categories` WHERE (`subdivision_id` = '$subdivision_id') ORDER BY `category_name` ASC";
	$qu_inv_05_categories_EXE = mysqli_query($KONN, $qu_inv_05_categories_sel);
	if(mysqli_num_rows($qu_inv_05_categories_EXE)){
		$CC = 0;
		while($inv_05_categories_REC = mysqli_fetch_assoc($qu_inv_05_categories_EXE)){
			$category_id = $inv_05_categories_REC['category_id'];
			
			
		?>
			<div class="box-view" id="boxdata-<?=$category_id; ?>">
				<span class="id"><?=$category_id; ?></span>
				<span class="iconer"><?=$inv_05_categories_REC["category_name"]; ?></span>
				<div class="opt">
					<a href="inv_lvl06.php?ids=<?=$category_id; ?>"><i class="fas fa-folder-open"></i></a>
					<span class="box-sep"></span>
					<a onclick="edit_data(<?=$category_id; ?>);"><i class="fas fa-edit"></i></a>
					<span class="box-sep"></span>
					<a onclick="del_data(<?=$category_id; ?>);"><i class="fas fa-trash"></i></a>
				</div>
			</div>
		<?php
...
?>
			...
			<div class="box-view" id="family-0" onclick="add_new_modal_inv_05_categories();" style="opacity:0.7;">
				<span class="id">&nbsp;</span>
...
<!--    ///////////////////      add_new_modal_inv_05_categories Modal START    ///////////////////            -->
<div class="modal" id="add_new_modal_inv_05_categories">
	<div class="modal-container">
		...
		<div class="modal-body">
				...
				<form 
				id="add-new-inv05-form" 
				id-modal="add_new_modal_inv_05_categories" 
				class="boxes-holder" 
				api="<?=api_root; ?>inventory/lvl05/add_new.php">
...
<input class="frmData" type="text" 
		id="new-category_name" 
		name="category_name" 
		req="1" 
		den="" 
		alerter="<?=lang("Please_Check_Category_Name", "AAR"); ?>">
	</div>
</div>
...
<div class="viewerBodyButtons">
		<button type="button" onclick="submit_form('add-new-inv05-form', 'reload_page');">
			<?=lang('Add', 'ARR', 1); ?>
		</button>
...
<!--    ///////////////////      add_new_modal_inv_05_categories Modal END    ///////////////////            -->
...
<!--    ///////////////////      edit_modal_inv_05_categories Modal START    ///////////////////            -->
<div class="modal" id="edit_modal_inv_05_categories">
	<div class="modal-container">
		<div class="modal-header">
			<?php include("app/modal_header.php"); ?>
		</div>
		<div class="modal-body">
				...
				<form 
				id="edit-inv05-form" 
				id-modal="edit_modal_inv_05_categories" 
				class="boxes-holder" 
				api="<?=api_root; ?>inventory/lvl05/edit_data.php">
...
<input class="frmData" type="hidden" 
		...
		req="1" 
		den="0" 
		alerter="<?=lang("Please_Check_category", "AAR"); ?>">
...
<div class="viewerBodyButtons">
		<button type="button" onclick="submit_form('edit-inv05-form', 'reload_page');">
			<?=lang('Save', 'ARR', 1); ?>
		</button>
		<button type="button" onclick="hide_modal();">
			<?=lang('Cancel', 'ARR', 1); ?>
...
<!--    ///////////////////      edit_modal_inv_05_categories Modal END    ///////////////////            -->
...
function del_data( ids_id ){
	...
	if( aa == true ){
		...
		$.ajax({
			url      :"<?=api_root; ?>inventory/lvl05/rem_data.php",
...
function edit_data( ids_id ){
	...
	$.ajax({
		url      :"<?=api_root; ?>inventory/lvl05/get_data.php",
		data     :{'typo': 'pc_call', 'ids_id': ids_id},
		dataType :"JSON",
		type     :'POST',
		success  :function(response){
			
$('#edit-category_id').val(response[0].category_id);
$('#edit-category_name').val(response[0].category_name);
$('#edit-category_description').val(response[0].category_description);
...
$('#edit-unit_id').val(response[0].unit_id);
			...
			show_modal( 'edit_modal_inv_05_categories' , titler );
...
function add_new_modal_inv_05_categories(){
	var titler = '<?=lang("Add_New_Entry", "AAR"); ?>';
	titler = titler + ' :: ' + '<?=$family_name; ?> - <?=$section_name; ?> - <?=$division_name; ?> - <?=$subdivision_name; ?>' ;
	show_modal( 'add_new_modal_inv_05_categories' , titler );
}
