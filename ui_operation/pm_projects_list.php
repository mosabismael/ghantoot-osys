<?php
	...
	if( isset( $_GET['ref'] ) ){
		...
		$SERCHCOND = " AND (`job_order_id` = '$thsREF')";
	}
...
function bindData( response ){
	...
	for( i=0 ; i < response.length ; i ++ ){
		...
		var tr = '' + 
			...
			'	<div class = "td" ><span id="joREF-'+response[i].job_order_id+'" class="text-primary cursored">' + response[i].job_order_ref + '</span></div>' + 
			...
			'	<div class = "td">' + 
//			'		<button type="button" onclick="showJoDetails(' + response[i].job_order_id + ", '" + response[i].job_order_ref  + "'" +  ", 'viewJoDetails'" + ');"><?=lang("BreakDown", "AAR"); ?></button>' + 
			'		<a href="pm_projects_breakdown.php?job_order_id=' + response[i].job_order_id + '"><button type="button"><?=lang("BreakDown", "AAR"); ?></button></a>' + 
			'		<a href="pm_projects_details.php?job_order_id=' + response[i].job_order_id + '"><button type="button"><?=lang("Details", "AAR"); ?></button></a>' + 
			'	</div>' + 
			'</div>';
		$('#tableBody').append( tr );
	}
...
<div class="DetailsViewer" id="viewJoDetails">
	<div class="viewerContainer">
		...
		<div class="viewerBody">
			<?php include('../forms/job_orders/view_jo_breakdown.php'); ?>
		</div>
	</div>
</div>
