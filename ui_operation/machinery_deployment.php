		...
		<div class="row">
			...
			<div class="col-100">
				...
				<table class="tabler" border="2">
					<thead>
						<tr>
							<th colspan="2" style="width:40%;" id="newTitle" rowspan="2"><?=lang("Project_Name", "AAR"); ?></th>
							<th style="width:20%;" rowspan="2"><?=lang("Task Name", "AAR"); ?></th>
					...
					<tbody>
						...
						<?php
							...
							$total_HIRE_TT = 0;
							//manhour
							$qu_job_orders_timesheets_sel = "SELECT * FROM  `job_orders_timesheets_machines` timesheet , job_orders_timesheets_machines_recs recs, job_orders_processes_acts_tasks task WHERE task.task_id = recs.task_id and timesheet.timesheet_id = recs.timesheet_id  and timesheet.job_order_id = $job_order_id ";
							if($ts_date_from != "" && $ts_date_to != ""){
								$qu_job_orders_timesheets_sel = "SELECT * FROM  `job_orders_timesheets_machines` timesheet , job_orders_timesheets_machines_recs recs, job_orders_processes_acts_tasks task WHERE task.task_id = recs.task_id and timesheet.timesheet_id = recs.timesheet_id and  timesheet.job_order_id = $job_order_id and date_from >= '$ts_date_from' and date_to <= '$ts_date_to' " ;
							}
							else if($ts_date_from != ""){
								$qu_job_orders_timesheets_sel = "SELECT * FROM  `job_orders_timesheets_machines` timesheet , job_orders_timesheets_machines_recs recs, job_orders_processes_acts_tasks task WHERE task.task_id = recs.task_id and timesheet.timesheet_id = recs.timesheet_id and timesheet.job_order_id = $job_order_id and date_from >= '$ts_date_from' " ;
							}
							else if($ts_date_to != ""){
								$qu_job_orders_timesheets_sel = "SELECT * FROM  `job_orders_timesheets_machines` timesheet , job_orders_timesheets_machines_recs recs, job_orders_processes_acts_tasks task WHERE task.task_id = recs.task_id and timesheet.timesheet_id = recs.timesheet_id and timesheet.job_order_id = $job_order_id and date_to <= '$ts_date_to' " ;
							}
							...
							if(mysqli_num_rows($qu_job_orders_timesheets_EXE)){
								while($job_orders_timesheets_recs_REC = mysqli_fetch_assoc($qu_job_orders_timesheets_EXE)){
									...
									$machine = $job_orders_timesheets_recs_REC['machine_id'];
									$task_name = $job_orders_timesheets_recs_REC['task_name'];
									...
									if($is_local == 1){
										...
										$total_HIRE_RT = $total_HIRE_RT + $HIRE_RT;
										$total_HIRE_OT = $total_HIRE_OT + $HIRE_OT;
										$total_HIRE_TT = $total_HIRE_TT + $HIRE_TT;
									}
