...
<?php
	$sNo = 0;
	$qu_punch_list_sel = "SELECT * FROM  `punch_list` WHERE `punch_status` = 'check' ";
	...
	if(mysqli_num_rows($qu_punch_list_EXE)){
		...
		?>
			<tr id="boxdata-<?=$punch_id; ?>">
				...
				<td><?=$check_date; ?></td>
				<td>
					<button type="button" style="padding: 5px;" onclick="do_data(<?=$punch_id; ?>);">Finish</button>
...
<div class="col-100">
	<div class="form-grp">
        ...
        <textarea class="frmData" 
        		...
        		req="1" 
        		den="" 
        		alerter="<?=lang("Please_Check_punch_txt", "AAR"); ?>"></textarea>
	</div>
</div>
...
function do_data( ids_id ){
	...
	if( aa == true ){
		...
		$.ajax({
			url      :"<?=api_root; ?>punch_list/close_data.php",
