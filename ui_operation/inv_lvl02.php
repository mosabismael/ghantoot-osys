<?php
	...
	if( !isset( $_GET['ids'] ) ){
		...
		$family_id = (int) test_inputs( $_GET['ids'] );
	}
...
<?php
	$qu_inv_02_sections_sel = "SELECT * FROM  `inv_02_sections` WHERE `family_id` = '$family_id' ORDER BY `section_name` ASC";
	$qu_inv_02_sections_EXE = mysqli_query($KONN, $qu_inv_02_sections_sel);
	if(mysqli_num_rows($qu_inv_02_sections_EXE)){
		$CC = 0;
		while($inv_02_sections_REC = mysqli_fetch_assoc($qu_inv_02_sections_EXE)){
			$section_id = $inv_02_sections_REC['section_id'];
			
		?>
			<div class="box-view" id="boxdata-<?=$section_id; ?>">
				<span class="id"><?=$section_id; ?></span>
				<span class="iconer"><?=$inv_02_sections_REC["section_name"]; ?></span>
				<div class="opt">
					<a href="inv_lvl03.php?ids=<?=$section_id; ?>"><i class="fas fa-folder-open"></i></a>
					<span class="box-sep"></span>
					<a onclick="edit_data(<?=$section_id; ?>);"><i class="fas fa-edit"></i></a>
					<span class="box-sep"></span>
					<a onclick="del_data(<?=$section_id; ?>);"><i class="fas fa-trash"></i></a>
				</div>
			</div>
		<?php
...
?>
			...
			<div class="box-view" id="family-0" onclick="add_new_modal_inv_02_sections();" style="opacity:0.7;">
				<span class="id">&nbsp;</span>
...
<!--    ///////////////////      add_new_modal_inv_02_sections Modal START    ///////////////////            -->
<div class="modal" id="add_new_modal_inv_02_sections">
	...
	<div class="modal-container">
		...
		<div class="modal-body">
				...
				<form 
				id="add-new-inv02-form" 
				id-modal="add_new_modal_inv_02_sections" 
				class="boxes-holder" 
				api="<?=api_root; ?>inventory/lvl02/add_new.php">
...
<input class="frmData" type="hidden" 
		...
		req="1" 
		den="0" 
		alerter="<?=lang("Please_Check_Family", "AAR"); ?>">
...
<input class="frmData" type="text" 
		...
		req="1" 
		den="" 
		alerter="<?=lang("Please_Check_Section_Name", "AAR"); ?>">
	</div>
</div>
...
<div class="viewerBodyButtons">
		<button type="button" onclick="submit_form('add-new-inv02-form', 'reload_page');">
			<?=lang('Add', 'ARR', 1); ?>
		</button>
...
<!--    ///////////////////      add_new_modal_inv_02_sections Modal END    ///////////////////            -->




<!--    ///////////////////      edit_modal_inv_02_sections Modal START    ///////////////////            -->
<div class="modal" id="edit_modal_inv_02_sections">
	<div class="modal-container">
		<div class="modal-header">
			<?php include("app/modal_header.php"); ?>
		</div>
		<div class="modal-body">
				...
				<form 
				id="edit-inv02-form" 
				id-modal="edit_modal_inv_02_sections" 
				class="boxes-holder" 
				api="<?=api_root; ?>inventory/lvl02/edit_data.php">
...
<input class="frmData" type="hidden" 
		...
		req="1" 
		den="0" 
		alerter="<?=lang("Please_Check_Data", "AAR"); ?>">
...
<div class="viewerBodyButtons">
		<button type="button" onclick="submit_form('edit-inv02-form', 'reload_page');">
			<?=lang('Save', 'ARR', 1); ?>
		</button>
		<button type="button" onclick="hide_modal();">
			<?=lang('Cancel', 'ARR', 1); ?>
...
<!--    ///////////////////      edit_modal_inv_02_sections Modal END    ///////////////////            -->
...
function del_data( ids_id ){
	...
	if( aa == true ){
		...
		$.ajax({
			url      :"<?=api_root; ?>inventory/lvl02/rem_data.php",
...
function edit_data( ids_id ){
	...
	$.ajax({
		url      :"<?=api_root; ?>inventory/lvl02/get_data.php",
		...
		success  :function(response){
			...
			$('#edit-section_code').val(response[0].section_code);
			$('#edit-section_name').val(response[0].section_name);
			$('#edit-section_description').val(response[0].section_description);
			...
			show_modal( 'edit_modal_inv_02_sections' , titler );
...
function add_new_modal_inv_02_sections(){
	var titler = '<?=lang("Add_New_Entry", "AAR"); ?>';
	titler = titler + ' :: ' + '<?=$family_name; ?>' ;
	show_modal( 'add_new_modal_inv_02_sections' , titler );
}
