<?php
	...
	$showPerPage = 20;
	$totPages = 0;
	$qu_pur_requisitions_sel = "SELECT COUNT(`requisition_id`) FROM  `pur_requisitions`  WHERE ( (`requisition_status` = 'draft') AND (`ordered_by` = '$EMPLOYEE_ID') $SERCHCOND )";
...
<html dir="<?=$lang_dir; ?>" lang="<?=$lang; ?>">
	...
	<body>
					...
					<script>
						...
						function bindData( response ){
							...
							for( i=0 ; i < response.length ; i ++ ){
								...
								'	<div class = "td">' + 
								'		<button type="button" onclick="archive_req(' + response[i].requisition_id + ');"><?=lang("Archive", "AAR"); ?></button>' + 
								'		<button type="button" onclick="activate_req(' + response[i].requisition_id + ');"><?=lang("Approve", "AAR"); ?></button>' + 
								'		<button type="button" onclick="remove_req(' + response[i].requisition_id + ');"><?=lang("Delete", "AAR"); ?></button>' + 
								'	</div>' + 
								'</div>';
								$('#tableBody').append( tr );
							}
		...
		<script>
			...
			function archive_req( IDD ){
				var aa = confirm( 'Are you sure, this action cannot be undo ?' );
				if( aa == true ){
					start_loader('Sending Requisition...');
					$.ajax({
						url      :"<?=api_root; ?>requisitions/archive_req.php",
						data     :{ 'requisition_id': IDD },
						dataType :"html",
						type     :'POST',
						success  :function(data){
						...
						for( i=0 ; i < response.length ; i ++ ){
							...
							if( req_item_id != 0 ){
								...
								*/
								...
								var reqOpts = '';
								if( ReqStatus == 'draft' ){
									reqOpts = '<i title="Edit this item" onclick="editReqItem('+req_item_id+');" class="fas fa-edit"></i>'+
									'<i title="Delete this item" onclick="delReqItem(' + req_item_id + ');" class="fas fa-trash"></i>';
			...
			function family(family_id , section_id , division_id , subdivision_id , category_id , item_code_id){
				$.ajax({
					url      :"<?=api_root; ?>inventory/lvl02/get_dt.php",
					data     :{ 'data_id': family_id , 'section_id': section_id},
					dataType :"html",
					type     :'POST',
					success  :function(data){
						//clear all others
						...
						section(section_id , division_id , subdivision_id , category_id , item_code_id);
					},
					error    :function(){
						alert('Data Error No: 5467653');
					},
			...
			function section(section_id , division_id , subdivision_id , category_id , item_code_id){
				
				$.ajax({
					url      :"<?=api_root; ?>inventory/lvl03/get_dt.php",
					data     :{ 'data_id': section_id  , 'division_id':division_id},
					dataType :"html",
					type     :'POST',
					success  :function(data){
						$('#f-division_id').show();
						...
						division(division_id , subdivision_id , category_id , item_code_id);
					},
					error    :function(){
						alert('Data Error No: 5467653');
					},
			...
			function division( division_id , subdivision_id , category_id , item_code_id){
				$.ajax({
					url      :"<?=api_root; ?>inventory/lvl04/get_dt.php",
					data     :{ 'data_id': division_id , 'subdivision_id' :subdivision_id },
					dataType :"html",
					type     :'POST',
					success  :function(data){
						...
						subdivision(subdivision_id , category_id , item_code_id);
					},
					error    :function(){
						alert('Data Error No: 5467653');
					},
			...
			function subdivision(subdivision_id , category_id , item_code_id){
				var dataID = parseInt( $('#new-subdivision_id').val() );
				var dataID1 = parseInt( $('#new-category_id').val() );
				$.ajax({
					url      :"<?=api_root; ?>inventory/lvl05/get_dt.php",
					data     :{ 'data_id': subdivision_id, 'category_id' :category_id },
					dataType :"html",
					type     :'POST',
					success  :function(data){
						$('#f-category_id').show();
						...
						category( category_id , item_code_id);
					},
					error    :function(){
						alert('Data Error No: 5467653');
					},
			...
			function category( category_id , item_code_id){
				var dataID = parseInt( $('#new-category_id').val() );
				var dataID1 = parseInt( $('#new-item_code_id').val() );
				$.ajax({
					url      :"<?=api_root; ?>inventory/lvl06/get_dt.php",
					data     :{ 'data_id': category_id, 'item_code_id' : item_code_id },
					dataType :"html",
					type     :'POST',
					success  :function(data){
						$('#f-item_code_id').show();
						...
						if( item_code_id != 0 ){
							var codeDesc = $('#coder-' + item_code_id).attr('desc');
							var codeUnit = parseInt( $('#coder-' + item_code_id).attr('uniter') );
			...
			function editReqItem( itmID ){
				...
				if( itmID != 0 ){
					...
					start_loader('Deleting Requisition Items...');
					$.ajax({
						url      :"<?=api_root; ?>requisitions/items/get_requisition_items.php",
						data     :{ 'req_item_id': itmID },
						dataType :"json",
						type     :'POST',
						success  :function(data){
							end_loader();
							show_details('NewItemDetails', 'Add_New_Item');
							
							for( i=0;i<data.length;i++){
								$('#new-family_id').val(data[i].family_id);
								$('#new-section_id').val(data[i].section_id);
								$('#news-division_id').val(data[i].division_id);
								$('#new-subdivision_id').val(data[i].subdivision_id);
								$('#new-category_id').val(data[i].category_id);
								$('#new-item_code_id').val(data[i].item_code_id);
								
								$('#new-item_qty').val(data[i].item_qty);
								$('#new-requisition_item_id').val(itmID);
								if(data[i].certificate_required != "NO"){
									$('#new-certificate_required').prop("checked",true);
									$('#new-item_cr').prop('disabled', false);
									$('#new-item_cr').attr('req', '1');
									$('#new-item_cr').val(data[i].certificate_required);
								}
								else{
									$('#new-certificate_required').prop("checked",false);
									$('#new-item_cr').prop('disabled', true);
									$('#new-item_cr').attr('req', '0');
									$('#new-item_cr').val('');
								}
								family(data[i].family_id ,data[i].section_id ,data[i].division_id, data[i].subdivision_id ,data[i].category_id ,data[i].item_code_id  );
			...
			function loadComparisonSheet( ITM ){
				...
				$.ajax({
					...
					data     :{ 'requisition_id': activeREQ, 'req_item_id': ITM, 'is_print': 1 },
					dataType :"html",
					type     :'POST',
					success  :function( response ){
				...
				<div class="viewerBody">
					...
					<div class="row col-33">
						...
						<div class="nwFormGroup">
							...
							<textarea id="savedNotes" class="requisition_notes" rows="6"></textarea><br>
							<button onclick="saveRegNotes();" type="button"><?=lang("Save_Notes", "AAR"); ?></button>
						</div>
					</div>
					<script>
						function saveRegNotes(){
							
							var nwNote = $('#savedNotes').val();
							if( nwNote != '' ){
								start_loader("Saving Notes...");
								$.ajax({
									url      :"<?=api_root; ?>requisitions/save_notes.php",
									data     :{ 'requisition_id': activeREQ, 'requisition_notes': nwNote },
									dataType :"html",
									type     :'POST',
									success  :function( response ){
										var resAr = response.split('|');
										var resAr0 = parseInt( resAr[0] );
										if(resAr0 != 0){
											end_loader();
											alert('Notes Saved');
										}
	...
	<?php include('../forms/requisitions/add_new_requisition_op.php'); ?>
	</div>
	</div>
	<div class="zero"></div>
	</div>
