<?php
	...
	$showPerPage = 20;
	$totPages = 0;
	$qu_pur_requisitions_sel = "SELECT COUNT(`requisition_id`) FROM  `pur_requisitions`  WHERE ( (`requisition_status` = 'archive') AND (`ordered_by` = '$EMPLOYEE_ID') $SERCHCOND )";
...
<html dir="<?=$lang_dir; ?>" lang="<?=$lang; ?>">
	...
	<body>
					...
					<script>
						...
						function bindData( response ){
							...
							for( i=0 ; i < response.length ; i ++ ){
								...
								'	<div class = "td">' + 
								'		<button type="button" onclick="draft_req(' + response[i].requisition_id + ');"><?=lang("Move to Draft", "AAR"); ?></button>' + 
								'	</div>' + 
								'</div>';
								$('#tableBody').append( tr );
							}
		...
		<script>
			...
			function draft_req( IDD ){
				var aa = confirm( 'Are you sure, this action cannot be undo ?' );
				if( aa == true ){
					start_loader('Sending Requisition...');
					$.ajax({
						url      :"<?=api_root; ?>requisitions/draft_ref.php",
						data     :{ 'requisition_id': IDD },
						dataType :"html",
						type     :'POST',
						success  :function(data){
