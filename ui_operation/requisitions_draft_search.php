<?php
	...
	if(isset($_REQUEST["term"]) && strlen($_REQUEST["term"]) >0){
		...
		//-------------------GET MY REQs-----------------
		$sql = "SELECT `$search_option` FROM `pur_requisitions`  WHERE ( (`requisition_status` <> 'deleted') AND (`$search_option` LIKE ? ) AND (`ordered_by` = '$EMPLOYEE_ID') ) AND `requisition_status` = 'draft' group by $search_option";
		
		if($search_option == 'job_order_id'){
			$sql = "SELECT * FROM `pur_requisitions` req, `job_orders` job  WHERE ( (`requisition_status` <> 'deleted') AND  ((job.job_order_ref LIKE ?) OR (job.project_name LIKE ?)) AND `requisition_status` = 'draft' AND(req.job_order_id = job.job_order_id  ) AND (`ordered_by` = '$EMPLOYEE_ID') ) group by job.$search_option";
		}
