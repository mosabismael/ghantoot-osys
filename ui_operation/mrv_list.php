<?php
	...
	$qu_pur_requisitions_sel = "SELECT COUNT(*) FROM  `inv_mrvs` WHERE ((`mrv_status` = 'inspection_required') AND ( `inspected_by` = '$EMPLOYEE_ID' ) )";
			...
			<div class="col-100">
				<div class="tableHolder">
					...
					<div class="table">
						<div class="tableHeader">
							<div class="tr">
								...
								<div class = "th"><?=lang("Inspected_By", "AAR"); ?></div>
					...
					<div class="tablePagination">
						<div class="pageNum arrowPager" id="prePatchBtn" onclick="showPrePageBatch('ui_operation/mrv_list.php', <?=$showPerPage; ?>, <?=$totPages; ?>);"><i class="fas fa-angle-double-left"></i></div>
						<?php
							for( $i=$page ; $i<$page+10 ; $i++ ){
								...
								if( $i <= $totPages ){
								?>
								<div onclick="loadTableData( <?=$i; ?>, <?=$showPerPage; ?>, 'ui_operation/mrv_list.php' );" class="pageNum imVarPager pn-<?=$i; ?> <?=$CLS; ?>"><?=$iView; ?></div>
								<?php
								}
							}
						?>
						<div id="addPagerPoint"></div>
						<div class="pageNum arrowPager" id="nextPatchBtn" onclick="showNextPageBatch('ui_operation/mrv_list.php', <?=$showPerPage; ?>, <?=$totPages; ?>);"><i class="fas fa-angle-double-right"></i></div>
					</div>
					...
					<script>
						...
						function bindData( response ){
							...
							for( i=0 ; i < response.length ; i ++ ){
								...
								var optionsValue = "";
								if(response[i].mrv_status=='inspection_required'){
									optionsValue = '<a href="mrv_approval.php?mrv_id='+response[i].mrv_id+'" class=""><button type="button"><?=lang("Start_Approvals", "AAR"); ?></button></a>'
								}
