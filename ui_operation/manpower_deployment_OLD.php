		...
		$timesheet_id = $job_orders_timesheets_REC['timesheet_id'];
			...
			if(mysqli_num_rows($qu_job_orders_timesheets_recs_EXE)){
				while($job_orders_timesheets_recs_REC = mysqli_fetch_assoc($qu_job_orders_timesheets_recs_EXE)){
					...
					$is_local     = (int) $job_orders_timesheets_recs_REC['is_local'];
					...
					if( $is_local == 1 ){
						...
						$RT_hire = $RT_hire + $regular_time;
						$OT_hire = $OT_hire + $ot_time;
						$TT_hire = $TT_hire + $total_time;
					}
...
?>
			<tr class="task" id="task-<?=$task_id; ?>" task="<?=$task_id; ?>" activity="<?=$activity_id; ?>">
				...
				<td><?=$task_name; ?></td>
				<td class="l_rt"><?=$RT; ?></td>
				<td class="h_rt"><?=$RT_hire; ?></td>
				<td class="l_ot"><?=$OT; ?></td>
				<td class="h_ot"><?=$OT_hire; ?></td>
				<td class="l_tt"><?=$TT; ?></td>
				<td class="h_tt"><?=$TT_hire; ?></td>
			</tr>
<?php
		}
	}
