<?php
	...
	try{
		if( isset($_POST['norm_act_name']) &&
			...
			$qu_gen_company_norms_sel = "SELECT * FROM  `gen_company_norms` WHERE `norm_act_name` = '$norm_act_name' ";
			...
			$qu_gen_company_norms_ins = "INSERT INTO `gen_company_norms` (
															`group_name`, 
															`process_name`, 
															`norm_act_name`, 
															`unit_id`, 
			...
			$insertStatement = mysqli_prepare($KONN,$qu_gen_company_norms_ins);
			
			mysqli_stmt_execute($insertStatement);
			
			$norm_id = mysqli_insert_id($KONN);
			if( $norm_id != 0 ){
				
				
				if( insert_state_change($KONN, "New-NORM", $norm_id, "gen_company_norms", $EMPLOYEE_ID) ) {
					die("1|Norm Added");
					} else {
