<?php
	...
	try{
		...
		$qu_purchase_orders_sel = "SELECT `employee_id` FROM  `purchase_orders` WHERE `po_id` = $po_id";
		...
		if(mysqli_num_rows($qu_purchase_orders_EXE)){
			...
			$created_by = ( int ) $purchase_orders_DATA['employee_id'];
		}
		...
		values ('pending_arrival', '$po_id is po approved', 'purchase_orders_draft.php', $EMPLOYEE_ID , $created_by, '$date_add'  , 0, $po_id, '');";
		$updateStatement = mysqli_prepare($KONN,$qu_pur_requisitions_updt);
		mysqli_stmt_execute($updateStatement);
		
		
		
		$approved_acc_by_date = date('Y-m-d H:i:00');
		
		$po_status = 'pending_arrival';
		...
		`approved_acc_by_date` = '".$approved_acc_by_date."', 
		`po_status` = '".$po_status."' 
		WHERE `po_id` = $po_id;";
