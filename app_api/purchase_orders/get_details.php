<?php
	...
	try{
		...
		$q = "update users_notifications set is_notified = 1 where po_id = $po_id and receiver_id = $EMPLOYEE_ID";
		
		$userStatement = mysqli_prepare($KONN,$q);
		mysqli_stmt_execute($userStatement);
		
		/*
		$q = "update purchase_orders set view_status = 1 where po_id = $po_id";
		$userStatement = mysqli_prepare($KONN,$q);
		mysqli_stmt_execute($userStatement);
		*/
		...
		$q = "SELECT * FROM  `purchase_orders` WHERE `po_id` = $po_id";
		$userStatement = mysqli_prepare($KONN,$q);
		mysqli_stmt_execute($userStatement);
