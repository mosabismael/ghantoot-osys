		...
		isset($_POST['media_description'])) {
			...
			if($booler!=0){
				...
				if(move_uploaded_file($fileTmpLoc, $targer_dir.$New_name.'.'.$extensiom)){
					...
					$qu = "INSERT INTO `acc_biling_media` ( 
					`file_path` ,
					`file_type` ,
					`file_size` ,
					`file_description` ,
					...
					if($sizer == 0 ){$sizer = 1;}
					mysqli_stmt_bind_param($stmtC, 'ssisiis', $db_file_name, $fileType, $sizer, $media_description, $bill_id, $EMPLOYEE_ID, $file_local_link);
					if(mysqli_stmt_execute($stmtC)){
						...
						if( $current_state_id != 0 ){
							...
							if( insert_state_change_dep($KONN, "Media_Inserted-".$db_file_name, $bill_id, "New_File_Uploaded-".$fileType, 'acc_biling_media', $EMPLOYEE_ID, $current_state_id) ){
