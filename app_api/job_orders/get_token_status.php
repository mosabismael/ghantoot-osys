<?php
	...
	<?php
		$qu_gen_status_change_sel = "SELECT * FROM  `gen_status_change` WHERE `item_type` = 'tkn_data' AND `item_id` = $token_id ORDER BY `status_id` ASC";
			...
			<?php
				...
				if(mysqli_num_rows($qu_gen_status_change_depandancy_EXE)){
					while($gen_status_change_depandancy_REC = mysqli_fetch_assoc($qu_gen_status_change_depandancy_EXE)){
						...
						$action_by = $gen_status_change_depandancy_REC['action_by'];
						$emp_name = get_emp_name($KONN, $action_by );
