		...
		isset($_POST['media_description'])) {
			...
			if($booler!=0){
				...
				if(move_uploaded_file($fileTmpLoc, $targer_dir.$New_name.'.'.$extensiom)){
					...
					if($sizer == 0 ){$sizer = 1;}
					mysqli_stmt_bind_param($stmtC, 'ssisii', $db_file_name, $fileType, $sizer, $media_description, $token_id, $EMPLOYEE_ID);
					if(mysqli_stmt_execute($stmtC)){
						...
						if( $current_state_id != 0 ){
							...
							if( insert_state_change_dep($KONN, "Media_Inserted-".$db_file_name, $token_id, "New_File_Uploaded-".$fileType, 'tkn_data_media', $EMPLOYEE_ID, $current_state_id) ){
