<?php
	...
	$sNo = $start + 1;
	$qu_purchase_orders_sel = "SELECT count(*)  as count , notification_title FROM  `users_notifications` where is_notified = 0  and requisition_id != 0 $serchCond group by is_notified;";
	...
	if(mysqli_num_rows($qu_purchase_orders_EXE)){
		while($purchase_orders_REC = mysqli_fetch_assoc($qu_purchase_orders_EXE)){
			...
			$requisition_status = $purchase_orders_REC['notification_title'];
