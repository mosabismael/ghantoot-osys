	...
	try{
		...
		isset($_POST['trn_no'])  
			...
			if( isset($_POST['client_address']) ){
				$client_address = test_inputs( $_POST['client_address'] );
			}
			
			$qu_gen_clients_updt = "UPDATE  `gen_clients` SET 
			...
			$updateStatement = mysqli_prepare($KONN,$qu_gen_clients_updt);
			mysqli_stmt_execute($updateStatement);
			// $client_id = mysqli_insert_id($KONN);
			if( $client_id != 0 ){
				
				if( insert_state_change($KONN, 'Client_information_updated', $client_id, "gen_clients", $EMPLOYEE_ID) ){
					die('1|Client Updated');
					} else {
