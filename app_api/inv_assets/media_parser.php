		...
		isset($_POST['media_description'])) {
			...
			if($booler!=0){
				...
				if(move_uploaded_file($fileTmpLoc, $targer_dir.$New_name.'.'.$extensiom)){
					...
					$qu = "INSERT INTO `inv_assets_media` ( 
					`file_path` ,
					`file_type` ,
					`file_size` ,
					`file_description` ,
					...
					if($sizer == 0 ){$sizer = 1;}
					mysqli_stmt_bind_param($stmtC, 'ssisiis', $db_file_name, $fileType, $sizer, $media_description, $asset_id, $EMPLOYEE_ID, $file_local_link);
					if(mysqli_stmt_execute($stmtC)){
						
						$current_state_id = get_current_state_id($KONN, $asset_id, 'acc_biling' );
						if( $current_state_id != 0 ){
							
							
							if( insert_state_change_dep($KONN, "Media_Inserted-".$db_file_name, $asset_id, "New_File_Uploaded-".$fileType, 'inv_assets_media', $EMPLOYEE_ID, $current_state_id) ){
