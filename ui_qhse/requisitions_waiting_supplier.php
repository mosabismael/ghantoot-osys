...
<?php
	$sNo = 0;
	$qu_pur_requisitions_sel = "SELECT * FROM  `pur_requisitions` WHERE ((`requisition_status` = 'waiting_supplier') AND (`ordered_by` = '$EMPLOYEE_ID')) ORDER BY `requisition_id` DESC";
	...
	if(mysqli_num_rows($qu_gen_status_change_EXE)){
		...
		?>
					<button type="button" onclick="activate_req(<?=$requisition_id; ?>);"><?=lang("Send_to_purchase_Dept.", "AAR"); ?></button>
		<?php
			}
		?>
				</td>
