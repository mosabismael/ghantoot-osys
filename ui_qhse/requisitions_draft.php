...
<?php
	...
	if(mysqli_num_rows($qu_gen_status_change_EXE)){
		...
		$is_material = ( int ) $pur_requisitions_REC['is_material'];
		$typo = "";
		if( $is_material == 1 ){
			$typo = "Material";
		} else {
		...
		?>
			<tr id="req-<?=$requisition_id; ?>">
				...
				<td id="reqStatus-<?=$requisition_id; ?>">
		<?php
			if( $requisition_status == 'draft' ){
		?>
					<button type="button" onclick="activate_req(<?=$requisition_id; ?>);"><?=lang("Approve.", "AAR"); ?></button>
