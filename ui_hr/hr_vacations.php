<?php
	...
	$showPerPage = 20;
	$totPages = 0;
	$qu_COUNT_sel = "SELECT count(*) FROM  `hr_employees_vacations`";
	...
	<html>
		...
		<body>
			...
			<div class="row">
							...
							function bindData( response ){
								...
								for( i=0 ; i < response.length ; i ++ ){
									...
									if( response[i].leave_status == 'draft' ){
										...
										button = '<a onclick="activateVacation('+response[i].vacation_id+');" id="act-'+response[i].vacation_id+'" title="<?=lang("Activate", "AAR"); ?>"><i class="fas fa-check"></i></a>'+
										'<a onclick="approveVacataion('+response[i].vacation_id+');" id="app-'+response[i].vacation_id+'" style="display:none;" title="<?=lang("Approve", "AAR"); ?>"><i class="fas fa-thumbs-up"></i></a>'+
										'<a onclick="denyVacation('+response[i].vacation_id+');" id="den-'+response[i].vacation_id+'" style="display:none;" title="<?=lang("Deny", "AAR"); ?>"><i class="fas fa-times"></i></a>';
										} else if( response[i].leave_status == 'pending_approval' ){
										button = '<a onclick="approveVacataion('+response[i].vacation_id+');" id="app-'+response[i].vacation_id+'" title="<?=lang("Approve", "AAR"); ?>"><i class="fas fa-thumbs-up"></i></a>'+
										'<a onclick="denyVacation('+response[i].vacation_id+');" id="den-'+response[i].vacation_id+'" title="<?=lang("Deny", "AAR"); ?>"><i class="fas fa-times"></i></a>';
									}
									
									
									var tr = '' + 
									'<tr id="vac-' + response[i].vacation_id + '">' + 
									'	<td> HRVAC-' + response[i].vacation_id + '</td>' + 
									'	<td class = "td cell-title"><a href="hr_employees_profile.php?employee_id='+response[i].employee_id+'&b=<?=basename($_SERVER['PHP_SELF']); ?>" style="color:blue;">'+response[i].employee_code+' - '+response[i].first_name+' '+response[i].last_name+'</a></td>'+ 
									'	<td>' + response[i].start_date + '</td>' + 
									'	<td>' + response[i].end_date + '</td>' + 
									'	<td class = "td stater">' + response[i].vacation_status + '</td>' + 
									'	<td class = "td text-center">' +
									'	<a href="print/hr_leaves_print.php?idd='+response[i].vacation_id+'" target="_blank" title="<?=lang("print", "AAR"); ?>"><i class="fas fa-print"></i></a>'+	
									'	<a onclick="deleteRecord('+response[i].vacation_id+');" title="<?=lang("Delete", "AAR"); ?>"><i style="color: red;" class="fas fa-trash-alt"></i></a>'+button+
									'	</td>' + 
									'</tr>';
									$('#tableBody').append( tr );
				...
				<!--    ///////////////////      add_new_vacation_modal Modal START    ///////////////////            -->
				<div class="modal" id="add_new_vacation_modal">
				...
				<form 
				id="new-employee-vacation-form" 
				id-modal="add_new_vacation_modal" 
				class="boxes-holder" 
				api="<?=api_root; ?>hr/employee_vacations_new.php">
				...
				id="new-vacation_date" 
				name="vacation_date" 
				req="1" 
				den="" 
				alerter="<?=lang("Please_Check_vacation_date ", "AAR"); ?>" value="<?=date('Y-m-d'); ?>" disabled>
				</div>
				</div>
				...
				<div class="col-50">
				<div class="form-grp">
				<label>Is Passport Released</label>
				<select class="frmData" 
				id="new-is_passport_released" 
				name="is_passport_released" 
				req="1" 
				den="" 
				alerter="<?=lang("Please_Check_is_passport_released", "AAR"); ?>">
				...
				<label>Status</label>
				<select class="frmData" 
				id="new-vacation_status" 
				name="vacation_status" 
				req="1" 
				den="" 
				alerter="<?=lang("Please_Check_vacation_status ", "AAR"); ?>" disabled>
				...
				<button type="button" onclick="submit_form('new-employee-vacation-form', 'reload_page');">
				<?=lang('Save', 'ARR', 1); ?>
				</button>
				...
				function add_new_vacation_modal(){
				var titler = '<?=lang("Add_New_vacation", "AAR"); ?>';
				show_modal( 'add_new_vacation_modal' , titler );
				}
