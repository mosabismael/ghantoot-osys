<?php
	...
	$qu_job_orders_timesheets_machines_sel = "SELECT * FROM  `job_orders_timesheets_machines` WHERE `timesheet_id` = $timesheet_id";
	$qu_job_orders_timesheets_machines_EXE = mysqli_query($KONN, $qu_job_orders_timesheets_machines_sel);
	$job_orders_timesheets_machines_DATA;
	if(mysqli_num_rows($qu_job_orders_timesheets_machines_EXE)){
		$job_orders_timesheets_machines_DATA = mysqli_fetch_assoc($qu_job_orders_timesheets_machines_EXE);
	}
		$timesheet_id = $job_orders_timesheets_machines_DATA['timesheet_id'];
		$job_order_id = $job_orders_timesheets_machines_DATA['job_order_id'];
		$ts_date = $job_orders_timesheets_machines_DATA['ts_date'];
		$created_date = $job_orders_timesheets_machines_DATA['created_date'];
		$created_by = $job_orders_timesheets_machines_DATA['created_by'];
...
?>
				...
				class="boxes-holder" 
				api="<?=api_root; ?>projects/machine_timesheets/edit_timesheet.php">
...
<div class="col-100" style="width:100%;">
			...
			<table class="tabler" style="width:100%;" border="1">
				<thead>
					<tr>
						<th><?=lang('No.'); ?></th>
						<th colspan="2"><?=lang('Machine_Name'); ?></th>
...
<?php
$task_CODER = "";
	$qu_job_orders_timesheets_machines_recs_sel = "SELECT * FROM  `job_orders_timesheets_machines_recs` WHERE `timesheet_id` = $timesheet_id";
	$qu_job_orders_timesheets_machines_recs_EXE = mysqli_query($KONN, $qu_job_orders_timesheets_machines_recs_sel);
	if(mysqli_num_rows($qu_job_orders_timesheets_machines_recs_EXE)){
		$sNO = 0;
		while($job_orders_timesheets_machines_recs_REC = mysqli_fetch_assoc($qu_job_orders_timesheets_machines_recs_EXE)){
			$sNO++;
			$record_id = $job_orders_timesheets_machines_recs_REC['record_id'];
			$machine_id = $job_orders_timesheets_machines_recs_REC['machine_id'];
			$date_from = $job_orders_timesheets_machines_recs_REC['date_from'];
			$time_from = $job_orders_timesheets_machines_recs_REC['time_from'];
			$date_to = $job_orders_timesheets_machines_recs_REC['date_to'];
			$time_to = $job_orders_timesheets_machines_recs_REC['time_to'];
			$regular_time = $job_orders_timesheets_machines_recs_REC['regular_time'];
			$ot_time = $job_orders_timesheets_machines_recs_REC['ot_time'];
			$total_time = $job_orders_timesheets_machines_recs_REC['total_time'];
			$task_id = $job_orders_timesheets_machines_recs_REC['task_id'];
...
<tr class="ts_record" id="rec-<?=$record_id; ?>" ids="<?=$record_id; ?>"  night="1">
	...
	<td colspan="2">
		<input class="frmData machine_id" ids="<?=$record_id; ?>" 
				id="new-machine_id<?=$record_id; ?>" 
				name="machine_ids[]" 
				value="<?=$machine_id; ?>" 
				type="text"
				req="1" 
				den="" 
				alerter="<?=lang("Please_Check_machine_code", "AAR"); ?>" style="width:100%;">
	</td>
...
<div class="btns-holder text-center">
	...
	<a href="machines_timesheets.php"><button class="btn btn-warning" type="button" onclick=""><?=lang('Back'); ?></button></a>
...
function addTsRec(){
	...
	var tr = 		'<tr class="ts_record" id="rec-' + totRecs + '" ids="' + totRecs + '"  night="1">' + 
					...
					'	<td>' + 
					'		<input class="frmData machine_code" list="machine_codes" ids="' + totRecs + '" ' + 
					'				id="new-machine_code' + totRecs + '" ' + 
					'				name="machine_codes[]" ' + 
					'				type="text"' + 
					'				req="1" ' + 
					'				den="" ' + 
					...
					'	</td>' + 
					'	<td>' + 
					'		<input class="frmData machine_id" ids="' + totRecs + '" ' + 
					'				id="new-machine_id' + totRecs + '" ' + 
					'				name="machine_ids[]" ' + 
					'				type="hidden"' + 
					'				req="1" ' + 
					'				den="" ' + 
					'				alerter="<?=lang("Please_Check_machine_code", "AAR"); ?>" style="width:100%;">' + 
					'		<input class="machine_name" id="new-machine_name' + totRecs + '" type="text" disabled style="width:100%;">' + 
					'	</td>' + 
					'	<td style="text-align: center;">' + 
...
function getEmpName( thsCode, thsIDDD ){
	...
	if( thsCode != '' ){
		...
		if( thsName != '' ){
			...
			if( empID != 0 ){
				$('#new-machine_name' + thsIDDD).val(thsName);
				$('#new-machine_id' + thsIDDD).val(empID);
			}
...
function initTableEvents(){
	$('.machine_code').on( 'input', function(){
		var thsID = $(this).val();
