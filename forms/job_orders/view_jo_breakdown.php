			...
			<div class="tabs">
				<div class="tabsHeader">
					<div onclick="set_tabber(4);loadGroups();" class="tabsIdSel-4 activeHeaderTab"><?=lang("Groups", "AAR"); ?></div>
					<div onclick="set_tabber(1);loadProcesses();" class="tabsIdSel-1"><?=lang("Processes", "AAR"); ?></div>
				...
				<div class="tabsId-4 tabsBody tabsBodyActive">
					<table class="tabler" border="1">
						<thead>
							<tr>
								<th style="width:5%;">---</th>
								<th style="width:5%;"><?=lang("NO", "AAR"); ?></th>
								<th style="width:50%;"><?=lang("Group", "AAR"); ?></th>
							</tr>
						</thead>
						<tbody id="jo_groups"></tbody>
					</table>
					<div class="viewerBodyButtons">
					</div>
				</div>
				
				<div class="tabsId-1 tabsBody">
					<table class="tabler" border="1">
						<thead>
							<tr>
								<th style="width:5%;">---</th>
...
function loadGroups(){
	start_loader('Loading Groups...');
	$.ajax({
		url      :"<?=api_root; ?>projects/groups/get_groups.php",
		data     :{ 'job_order_id': activeJO },
		dataType :"json",
		type     :'POST',
		success  :function( response ){
				end_loader();
				$('#new-act-group_id').html('');
				...
				for( i=0 ; i < response.length ; i ++ ){
					cc++;
					var group_id    = parseInt( response[i].group_id );
					if( group_id != 0 ){
						...
						var tr = '' + 
								...
								'	<td><i title="Delete this item" onclick="delGroup(' + group_id + ');" class="fas fa-trash"></i></td>' + 
								'	<td>' + cc + '</td>' + 
								'	<td>' + group_name + '</td>' + 
								'</tr>';
						$('#new-pro-gruop_id').append('<option value="' + group_id + '">' + group_name + '</option>');
						$('#new-task-group_id').append('<option value="' + group_id + '">' + group_name + '</option>');
						$('#jo_groups').append(tr);
					}
				}
