<?php
	...
	$showPerPage = 20;
	$totPages = 0;
	$qu_COUNT_sel = "SELECT count(*) FROM  `purchase_orders` WHERE ((`po_status` = 'generated') $SERCHCOND )";
				...
				<script>
					...
					function bindData( response ){
						...
						for( i=0 ; i < response.length ; i ++ ){
							...
							'	<div class = "td" onclick="showPoDetails('+response[i].po_id+ ",'" +response[i].po_ref+ "'" +",'viewPOdetails'"+');"><span id="poREF-'+response[i].po_id+'" class="text-primary">'+response[i].po_ref+'</span></div>'+
