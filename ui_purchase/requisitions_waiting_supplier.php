<?php
	...
	if(isset( $_GET['click'])){
		$click=1;
		$click_value = $_GET['click'];
	}
	$finish=0;
	$finish_value = "";
	if(isset( $_GET['finish'])){
		$finish=1;
		$finish_value = $_GET['finish'];
	}
	$page = 1;
	$showPerPage = 20;
	$totPages = 0;
	$qu_pur_requisitions_sel = "SELECT count(*) FROM  `pur_requisitions` WHERE ((`requisition_status` = 'waiting_supplier') $SERCHCOND )";
	...
	<body>
		...
		<div class="row">
					...
					<script>
						...
						function bindData( response ){
							...
							for( i=0 ; i < response.length ; i ++ ){
								...
								'	<div class = "td" id = "click-'+response[i].requisition_id+'" onclick="showReqDetails(' + response[i].requisition_id + ", '" + response[i].requisition_ref  + "'" +  ", 'viewItemDetails'" + ');"><span id="reqREF-'+response[i].requisition_id+'" class="text-primary">' + response[i].requisition_ref + '</span></div>' + 
							...
							if(<?=$click?>==1){
								document.getElementById("click-<?=$click_value?>").click();
							}
							if(<?=$finish?>==1){
								document.getElementById("click-<?=$finish_value?>").click();
								set_tabber(6);loadApprovedSupps();
								$('#nextButton').css( 'pointer-events', 'none' );
								$('#nextButton').css( 'background', 'var(--greyLight)' );
								$('#nextButton').css( 'color', 'var(--greyDark)' );
								$('#compatitive_supplier').removeClass('inactiveButton');
			...
			<script>
				...
				function finish_req( IDD ){
					var aa = confirm( 'Are you sure, this action cannot be undo ?' );
					if( aa == true && activeREQ != 0 ){
						start_loader('Sending Requisition...');
						$.ajax({
							url      :"<?=api_root; ?>requisitions/finish_req.php",
							data     :{ 'requisition_id': activeREQ },
							dataType :"html",
							type     :'POST',
							success  :function(data){
				...
				function nextButton(){
					if($('#compatitive_supplier').css('pointer-events') == 'auto' && $('#comparision_sheet').css('pointer-events') == 'auto' && $('#compatitive_supplier').css('background-color') == 'rgb(147, 183, 215)' ){
						finish_req();
					}
					else if($('#compatitive_supplier').css('pointer-events') == 'auto'){
						set_tabber(6);loadApprovedSupps();
						$('#nextButton').css( 'pointer-events', 'none' );
						$('#nextButton').css( 'background', 'var(--greyLight)' );
						$('#nextButton').css( 'color', 'var(--greyDark)' );
					...
					else if($('#comparision_sheet').css('pointer-events') == 'auto'){
						$('#nextButton').css( 'pointer-events', 'none' );
						$('#nextButton').css( 'background', 'var(--greyLight)' );
						$('#nextButton').css( 'color', 'var(--greyDark)' );
						$('#alert').css( 'display', 'none' );
						set_tabber(1);loadReqItems();
				...
				function checkapproved(req_item_id, activeREQ){
					$.ajax({
						url      :"<?=api_root; ?>requisitions/rfqs/check_approved.php",
						data     :{ 'requisition_id': activeREQ, 'req_item_id':req_item_id },
						dataType :"json",
						type     :'POST',
						success  :function( response ){
							if(response[0].is_approved == true){
								$('#reqItem-'+req_item_id).css('background','var(--bs-success)');
						...
						success  :function( response ){
							...
							for( i=0 ; i < response.length ; i ++ ){
								...
								if( req_item_id != 0 ){
									...
									*/
									checkapproved(req_item_id, activeREQ);									
				...
				function delete_rfq( rfqID ){
					var aa = confirm('Are you sure, this will clear the inserted price list, action cannot be undone ?');
					if( aa == true ){
						
						start_loader("Removing RFQ...");
						$.ajax({
							url      :"<?=api_root; ?>requisitions/rfqs/remove_rfq.php",
							data     :{ 'rfq_id': rfqID },
							dataType :"html",
							type     :'POST',
							success  :function( response ){
				...
				function delete_pl_detail( plID ){
					var aa = confirm('Are you sure, this will clear the inserted price list, action cannot be undone ?');
					if( aa == true ){
						
						start_loader("Removing Price List...");
						$.ajax({
							url      :"<?=api_root; ?>requisitions/price_lists/remove_pl.php",
							data     :{ 'price_list_id': plID },
							dataType :"html",
							type     :'POST',
							success  :function( response ){
								//split response
					...
					$.ajax({
						...
						success  :function( response ){
							...
							for( i=0 ; i < response.length ; i ++ ){
								...
								if( rfq_id != 0 ){
									...
									if( price_list_id == 0 ){
										...
										rfqOpt = '<a title="Remove Supplier Price" onclick="delete_pl_detail(' + price_list_id + ');"><i class="fas fa-trash" style="color:red;"></i></a>' + rfqOpt;
									...
									var tr = '' + 
									'<tr id="rfq-' + rfq_id + '">' + 
									'	<td><a title="Remove RFQ" onclick="delete_rfq(' + rfq_id + ');"><i class="fas fa-times" style="color:red;"></i></a></td>' + 
									'	<td>' + cc + '</td>' + 
							...
							if(completed == 1){
								...
								$('#nextButton').css( 'background-color', '#d3e4f4' );
								$('#nextButton').css( 'color', 'black' );
								$('#comparision_sheet').removeClass('inactiveButton');
				...
				function loadRfqDetails( TT ){
					...
					$.ajax({
						...
						type     :'POST',
						success  :function( response ){
							$('#addPriceListDetails').find('.element-to-paste-filename').text('');
							...
							if( price_list_id != 0 ){
								...
								$('#addPriceListDetails #pl-new-is_vat_included').val(response[0].is_vat_included);
								$('#addPriceListDetails #viewQuoteSupp').attr('href', '../uploads/' + response[0].attached_supplier_quotation);
								$('#addPriceListDetails').find('.element-to-paste-filename').text(response[0].attached_supplier_quotation+","+response[0].attached_supplier_quotation1);
								//$('#addPriceListDetails #pl-new-attached_supplier_quotation').val(response[0].attached_supplier_quotation);
								//$('#addPriceListDetails #pl-new-attached_supplier_quotation1').val(response[0].attached_supplier_quotation1);
				...
				function checkActivateNextbutton(){
					var isapproved = 1;
					var inside =0;
					$.each($('#req_items').find('tr'), function() {
						inside =1;
						col1Val = $(this).css('background-color');
						if(col1Val == 'rgb(76, 174, 76)'){
						}
						else if (typeof col1Val === "undefined"){
							isapproved=0;
						}
						else{
							isapproved = 0;
				...
				function comparisonSheet(){
					...
					if( selctedCount == 0 ){
						...
						alert("Select one item to View Comparison Sheet");
					}
						...
						$.ajax({
							...
							success  :function( response ){
								...
								if( res == 1 ){
									...
									hide_details('CSviewDetail');
									loadComparisonSheet( req_item_id );
									checkapproved(req_item_id, requisition_id);
									} else {
									alert( ARR[1] );
								}
					...
					<div class="viewerBody">
						...
						<div class="tabs">
							<div class="tabsHeader">
								<div onclick="set_tabber(5);loadRFQs();" class="tabsIdSel-5 activeHeaderTab"><?=lang("RFQs_List", "AAR"); ?></div>
								<div onclick="set_tabber(1);loadReqItems();" id = "comparision_sheet" class="tabsIdSel-1 inactiveButton"><?=lang("Comparison_Sheet", "AAR"); ?></div>
								<div onclick="set_tabber(6);loadApprovedSupps();" id = "compatitive_supplier" class="inactiveButton tabsIdSel-6"><?=lang("Compatative_Suppliers", "AAR"); ?></div>
						...
						<div class="viewerBodyButtons">
							<!--<button type="button" onclick="finish_req();"><?=lang("Finish_Requisition", "AAR"); ?></button>-->
							...
							<button type="button" id = "nextButton" onclick="nextButton();" ><?=lang("NEXT", "AAR"); ?></button>
			...
			<div class="DetailsViewer ViewerOnTop" id="CSviewDetail">
				<div class="viewerContainer">
					<div class="viewerHeader">
						<img src="<?=uploads_root; ?>/logo_icon.png" />
						<h1>REFREFREF</h1>
						<i onclick="hide_details('CSviewDetail');checkActivateNextbutton();" class="fas fa-times"></i>
					</div>
