<?php
	...
	$showPerPage = 20;
	$totPages = 0;
	$qu_COUNT_sel = "SELECT count(`po_id`) FROM  `purchase_orders` WHERE ((`po_status` = 'pending_arrival') $SERCHCOND )";
...
<html dir="<?=$lang_dir; ?>" lang="<?=$lang; ?>">
	...
	<body>
					...
					<script>
						...
						function bindData( response ){
							...
							for( i=0 ; i < response.length ; i ++ ){
								...
								'	<div class = "td">' + 
								'	<button title="Mail PO" onclick="openEmail('+response[i].po_id+')" target="_blank"><i class="fas fa-envelope"></i></button>'+
		...
		<script>
			...
			function openEmail( po_id ){
				var hrefLink = 'mailto:ahmad.f@ghantootomi.com?subject=PO-' + po_id + '&cc=procurement@ghantootomi.com&body=Good Day,%0D%0A%0D%0A%0D%0A%0D%0A' ;
				...
				if( po_id != 0 ){
					...
					$.ajax({
						...
						*/
						...
						<?php
						require_once('../forms/purchase_orders/view_po_details_approved.php');
						?>
						</div>
						</div>
						</div>
