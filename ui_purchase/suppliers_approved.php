<?php
	...
	$showPerPage = 20;
	$totPages = 0;
	$qu_pur_requisitions_sel = "SELECT count(`supplier_id`) FROM  `suppliers_list` WHERE `is_approved` = '1'";
				...
				<div class="tablePagination">
					...
					<?php
						for( $i=$page ; $i<$page+10 ; $i++ ){
							...
							var button = '<form method="POST">'+
							'<input type="hidden" name="supplier_id" value="'+response[i].supplier_id+'">'+
							'<input type="hidden" name="is_approve" value="0">'+
							'<button type="submit"><?=lang("Remove_From_Approved", "AAR"); ?></button>'+
							'</form>';
							if(response[i].is_approved == 0){
							button = '<form method="POST"><input type="hidden" name="supplier_id" value="'+response[i].supplier_id+'">'+
							'<input type="hidden" name="is_approve" value="1">'+
							'<button type="submit"><?=lang("Approved", "AAR"); ?></button>'+
							'</form>';
							}
