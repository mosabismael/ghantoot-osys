<?php
	...
	$showPerPage = 20;
	$totPages = 0;
	$qu_COUNT_sel = "SELECT count(*) FROM  `purchase_orders` WHERE ((`po_status` = 'draft') $SERCHCOND )";
	...
	<body>
		...
		<div class="row">
					...
					<div class="tableForm">
						<div class="tableFormGroup">
							<select id = "search_option">
								...
								<option value = "rev_no">Revision number</option>
							</select>
					...
					<div class="table">
						<div class="tableHeader">
							<div class="tr">
								...
								<div class="th"><?=lang("REVISION_NUMBER", "AAR"); ?></div>
					...
					<script>
						...
						function bindData( response ){
							...
							for( i=0 ; i < response.length ; i ++ ){
								...
								'	<div class = "td" id = "clickpo-'+response[i].po_id+'" onclick="showPoDetails('+response[i].po_id+ ",'" +response[i].po_ref+ "'" +",'viewPOdetails'"+');"><span id="poREF-'+response[i].po_id+'" class="text-primary">'+response[i].po_ref+'</span></div>'+
								'	<div class = "td">' + response[i].rev_no + '</div>' + 
							...
							if(<?=$click?>==1){
								document.getElementById("clickpo-<?=$click_value?>").click();
							}
						}
			...
			<script>
				...
				function old_revision_function(event){
					var rev_no = event.data.param1;
					var po_id = event.data.param2;
					var po_ref = event.data.param3;
					var viewPOdetails = event.data.param4;
					var org_rev_no = event.data.param5;
					...
					if( po_id != 0 ){
						...
						//load form details
						$.ajax({
							url      :"<?=api_root; ?>purchase_orders/get_details_from_revision.php",
							data     :{ 'po_id': po_id , 'rev_no': rev_no},
							dataType :"json",
							type     :'POST',
							success  :function( response ){
								...
								$('#' + viewPOdetails + ' .delivery_period_id').prop('disabled', true);
								
								$('#' + viewPOdetails + ' .delivery_period_id').val(response[0].delivery_period_id);
								...
								$('#' + viewPOdetails + ' .payment_term_id').val(response[0].payment_term_id);
								$('#' + viewPOdetails + ' .payment_term_id').prop('disabled', true);
								
								$('#' + viewPOdetails + ' .currency_id').val(response[0].currency_id);
								$('#' + viewPOdetails + ' .currency_id').prop('disabled', true);
								...
								if(response[0].rev_no>1){
									$('#oldVersion').css('display','block');
									$("#oldVersion").on('click',{param1: response[0].rev_no-1, param2: po_id, param3: po_ref , param4: viewPOdetails, param5: org_rev_no}, old_revision_function);
									$('#newVersion').css('display','block');
									$("#newVersion").on('click',{param1: org_rev_no, param2: po_id, param3: po_ref , param4: viewPOdetails}, new_revision_function);
								}
								else{
									$('#oldVersion').css('display','none');
									$('#newVersion').css('display','block');
									$("#newVersion").on('click',{param1: org_rev_no, param2: po_id, param3: po_ref , param4: viewPOdetails}, new_revision_function);
								}
								//load items
				...
				function new_revision_function(event){
					var rev_no = event.data.param1;
					var po_id = event.data.param2;
					var po_ref = event.data.param3;
					var viewPOdetails = event.data.param4;
					...
					if( po_id != 0 ){
						...
						$.ajax({
							...
							success  :function( response ){
								...
								$('#' + viewPOdetails + ' .po_date').val(response[0].po_date);
								$('#' + viewPOdetails + ' .delivery_date').val(response[0].delivery_date);
								$('#' + viewPOdetails + ' .delivery_period_id').prop('disabled', false);
								...
								$('#' + viewPOdetails + ' .is_vat_included').val(response[0].is_vat_included);
								$('#' + viewPOdetails + ' .payment_term_id').val(response[0].payment_term_id);
								$('#' + viewPOdetails + ' .payment_term_id').prop('disabled', false);
								$('#' + viewPOdetails + ' .currency_id').val(response[0].currency_id);
								$('#' + viewPOdetails + ' .currency_id').prop('disabled', false);
								$('#' + viewPOdetails + ' #cur_name_view').html(response[0].currency_name);
								$('#' + viewPOdetails + ' .exchange_rate').val(response[0].exchange_rate);
								$('#' + viewPOdetails + ' .supplier_quotation_ref').val(response[0].supplier_quotation_ref);
								
								$('#' + viewPOdetails + ' #viewQuoteSupp').attr('href', '../uploads/' + response[0].attached_supplier_quotation);
								$('#' + viewPOdetails + ' .supplier_id').prop('disabled', false);
								$('#' + viewPOdetails + ' .supplier_quotation_ref').prop('disabled', false);
								$('#' + viewPOdetails + ' .notes').val(response[0].notes);
								$('#' + viewPOdetails + ' .po_status').val(response[0].po_status);
								$('#' + viewPOdetails + ' .supplier_id').val(response[0].supplier_id);
								$('#' + viewPOdetails + ' .requisition_id').val(response[0].requisition_ref);
								...
								if(response[0].rev_no>1){
									$('#oldVersion').css('display','block');
									$("#oldVersion").on('click',{param1: response[0].rev_no-1, param2: po_id, param3: po_ref , param4: viewPOdetails, param5: response[0].rev_no}, old_revision_function);
			...
			<div class="DetailsViewer" id="viewPOdetails">
				<div class="viewerContainer">
					...
					<div class="viewerBody" id="po_view_details">
						<?php
							require_once('../forms/purchase_orders/view_po_details_drafts.php');
						?>
					</div>
				</div>
			</div>
			...
			<!--    ///////////////////      poDefineAppsDetails VIEW START    ///////////////////            -->
			<div class="DetailsViewer ViewerOnTop" id="poDefineAppsDetails">
				<div class="viewerContainer">
					<div class="viewerHeader">
						<img src="<?=uploads_root; ?>/logo_icon.png" />
						<h1>REFREFREF</h1>
						<i onclick="hide_details('poDefineAppsDetails');" class="fas fa-times"></i>
					</div>
					<div class="viewerBody">
						<?php include('../forms/purchase_orders/define_approvals.php'); ?>
					</div>
				</div>
			</div>
			<!--    ///////////////////      poDefineAppsDetails VIEW END    ///////////////////            -->
