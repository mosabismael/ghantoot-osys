<?php
	...
	$showPerPage = 20;
	$totPages = 0;
	$qu_pur_requisitions_sel = "SELECT count(*) FROM  `pur_requisitions` WHERE ((`requisition_status` = 'waiting_rfq') $SERCHCOND ) ";
	...
	<body>
		...
		<div class="row">
			...
			<script>
					...
					if( aa == true && activeREQ != 0 ){
						...
						$.ajax({
							...
							success  :function(data){
								...
								if( res == 1 ){
									//location.reload();
									window.location.href  = 'requisitions_waiting_supplier.php?click='+activeREQ;
				...
				function draft_again( IDD ){
					
					var reason = $('#rejection_notes').val();
					if( reason != '' ){
						var aa = confirm( 'Are you sure, this action cannot be undo ?' );
						if( aa == true && activeREQ != 0 ){
							start_loader('Sending Requisition...');
							$.ajax({
								url      :"<?=api_root; ?>requisitions/draft_again.php",
								data     :{ 'requisition_id': activeREQ, 'rejection_notes': reason },
								dataType :"html",
								type     :'POST',
								success  :function(data){
									end_loader();
							...
							for( i=0 ; i < response.length ; i ++ ){
								...
								if( req_item_id != 0 ){
									...
									*/
									...
									'	<td class="remAtCopy"><input type="checkbox" class="itemCheck" req_item="' + req_item_id + '" idd="reqItem-' + req_item_id + '"></td>' + 
									'	<td>' + cc + '</td>' + 
									'	<td>' + item_name + '</td>' + 
									'	<td class="remAtCopy">' + item_stock_qty + '</td>' + 
									'	<td>' + item_qty + '</td>' + 
									'	<td>' + item_days_v + '</td>' + 
									'	<td>' + item_unit_id + '</td>' + 
							...
							$.ajax({
								...
								success  :function(data){
									...
									if( res == 1 ){
										...
										$('#reqApprovedSup-'+rfq_item_id).remove();
										} else {
				...
				function generateRFQ(){
					...
					if( selctedCount != 0 ){
						...
						loadSelectedSuppliers(reqItem);
						//show supplierSelector
						
						
						} else {
						alert("Please Select Any 1 Items to generate RFQ");
					}
					
				}
				function loadSelectedSuppliers(reqItem){
					start_loader('Loading Suppliers List...');
					$.ajax({
						url      :"<?=api_root; ?>requisitions/rfqs/get_supplier_item_detail.php",
						data     :{ 'req_item_id': reqItem },
						dataType :"json",
						type     :'POST',
						success  :function( response ){
							end_loader();
							$('#rfq_supps_approved').html('');
							$('#selectedrfqItems').show();
							//load items
							var cc = 0;
							for( i=0 ; i < response.length ; i ++ ){
								cc++;
								...
								var supplier_id    = parseInt( response[i].supplier_id );
								if( supplier_id != 0 ){
									$('#reqSup-'+supplier_id).find('[type="checkbox"]').prop("disabled", true);
									var supplier_code= response[i].supplier_code;
									var supplier_name= response[i].supplier_name;
									var supplier_phone= response[i].supplier_phone;
									
									var tr = '' + 
									'<tr id="reqApprovedSup-' + response[i].rfq_item_id + '">' + 
									'	<td><i title="Delete this item" onclick="delPl_Item(' + response[i].rfq_item_id +');" class="fas fa-trash"></i></td>' + 
									'	<td>' + supplier_code + '</td>' + 
									'	<td>' + supplier_name + '</td>' + 
									'	<td>' + supplier_phone + '</td>' + 
									'</tr>';
									
									$('#rfq_supps_approved').append(tr);
								}
							}
						...
						error    :function(error){
							end_loader();
							alert('Data Error No: 5467653'+error);
						},
					});
					
				}
				function delPl_supplier(rfq_item_id){
				...
				function checkAllItemSupplierAdded(alert){
					$.ajax({
						url      :"<?=api_root; ?>requisitions/rfqs/check_all_item_supplier.php",
						data     :{ 'requisition_id': activeREQ },
						dataType :"json",
						type     :'POST',
						success  :function( response ){
							if(response[0].result=='true'){
								$('#nextButton').css( 'pointer-events', 'inherit' );
								$('#nextButton').css( 'background-color', '#d3e4f4' );
						...
						success  :function( response ){
							...
							for( i=0 ; i < response.length ; i ++ ){
								...
								if( rfq_id != 0 ){
									var supplier_code  = response[i].supplier_code;
									var supplier_name  = response[i].supplier_name;
									var supplier_email  = response[i].supplier_email;
									...
									var link = "mailto:"+supplier_email
									+ "?"
									...
									if( price_list_id == 0 ){
										rfqOpt = '<a title="print RFQ" href="prints/rfq_print.php?rfq_id=' + rfq_id + '" target="_blank"><i class="fas fa-print"></i></a>' + 
										'<a title="Send RFQ" href="' + link + '" target="_blank"><i class="fas fa-envelope"></i></a>'; 
										} else {
				...
				function loadRfqDetails( TT ){
					...
					$.ajax({
						...
						success  :function( response ){
							...
							$('#addPriceListDetails #pl-new-is_vat_included').val( '1' );
							...
							for( i=0 ; i < response.length ; i ++ ){
								...
								if( req_item_id != 0 ){
									...
									*/
									...
									'				req="1" ' + 
									'				den="0" ' + 
									'				alerter="<?=lang("Please_Check_item_no", "AAR"); ?>.' + cc + ' QTY">' + 
									'	</td>' + 
				...
				function emailRFQ(){
					...
					$('.itemCheck').each( function(){
						if ($(this).is(":checked")) {
							selctedCount++;
							// CS_item = $(this).attr('req_item') + '-' + CS_item;
							selItems = selItems + $(this).attr('req_item') + '-';
						}
					});
				...
				function openEmail( ITM ){
					//bring suppliers emails related to this item
					start_loader("Loading Comparison Sheet Details...");
					$.ajax({
						url      :"<?=api_root; ?>requisitions/get_supp_emails_req_item.php",
						data     :{ 'requisition_id': activeREQ, 'req_item_ids': ITM },
						dataType :"JSON",
						type     :'POST',
						success  :function( response ){
							end_loader();
							var item_name = " No              ITEM NAME                             ITEM QTY           UNIT      CERTIFICATE %0D%0A'";
							var emails = "";
							
							for( i=0 ; i < response.length ; i ++ ){
								var reqId    = parseInt( response[i].req_item_id );
								if( reqId != 0 ){
									var ths_name    = response[i].item_name;
									if( ths_name != '' ){
										item_name = item_name + ' ' + ths_name;
									}
								}
							}
							
							$.ajax({
								url      :"<?=api_root; ?>requisitions/get_supp_emails_req_item02.php",
								data     :{ 'requisition_id': activeREQ, 'req_item_ids': ITM },
								dataType :"JSON",
								type     :'POST',
								success  :function( response ){
									...
									for( i=0 ; i < response.length ; i ++ ){
										var supp    = parseInt( response[i].supplier_id );
										if( supp != 0 ){
											var mailer    = response[i].email;
											if( mailer != '' ){
									...
									var hrefLink = 'mailto:ahmad.f@ghantootomi.com?subject=RFQ-' + activeREQref + '&bcc=' + emails + '&cc=procurement@ghantootomi.com&body=Good Day,%0D%0A%0D%0AKindly provide us your best price rate for the below inquiry:%0D%0A%0D%0A' + item_name;
				...
				function copySelected(){
					...
					$('.itemCheck').each( function(){
						if ($(this).is(":checked")) {
							selctedCount++;
							thsItm = $(this).attr('req_item');
							thsBody += '<tr>' + $('#reqItem-' + thsItm).html() + '</tr>';
						}
					});
					...
					if( selctedCount == 0 ){
						...
						if( thsBody != '' ){
							//open new view and select table
							
							$('#copiedEmailItems').html( thsBody );
							setTimeout( function(){
								$('#copiedEmailItems .remAtCopy').each( function(){
									$(this).remove();
								} );
								show_details("copyEmailDetails", "copy Items");
								selectElementContents( document.getElementById('copierTable') );
								setTimeout( function(){
			...
			<div class="DetailsViewer" id="viewItemDetails">
				<div class="viewerContainer">
					<div id="alert">
						...
						<strong>Success!</strong> You can now proceed to next step by clicking NEXT button.
					</div>
					...
					<div class="viewerBody">
						...
						<div class="tabs">
							...
							<div class="tabsId-1 tabsBody tabsBodyActive">
								...
								<div class="viewerBodyButtons">
									...
									<!--<button type="button" onclick="emailRFQ();"><?=lang("Email_RFQ", "AAR"); ?></button>-->
									<button type="button" onclick="copySelected();"><?=lang("Copy_Selected", "AAR"); ?></button>
								</div>
							</div>
						...
						<div class="viewerBodyButtons">
							<!--<button type="button" onclick="waiting_supplier();"><?=lang("Finish_RFQs", "AAR"); ?></button>-->
							<button type="button" onclick="hide_details('viewItemDetails');"><?=lang("close", "AAR"); ?></button>
							<button type="button" onclick="draft_again();"><?=lang("Send_Back", "AAR"); ?></button>
							<button type="button" id = "nextButton" onclick="waiting_supplier();" ><?=lang("NEXT", "AAR"); ?></button>
						</div>
					</div>
				</div>
			</div>
			...
			<!--    ///////////////////      copyEmailDetails VIEW START    ///////////////////            -->
			<div class="DetailsViewer ViewerOnTop" id="copyEmailDetails">
				<div class="viewerContainer">
					<div class="viewerHeader">
						<img src="<?=uploads_root; ?>/logo_icon.png" />
						<h1>REFREFREF</h1>
						<i onclick="hide_details('copyEmailDetails');" class="fas fa-times"></i>
					</div>
					<div class="viewerBody">
						
						<table id="copierTable" class="tabler" border="1">
							<thead>
								<tr>
									<th><?=lang("NO", "AAR"); ?></th>
									<th><?=lang("Item", "AAR"); ?></th>
							...
							<tbody id="copiedEmailItems"></tbody>
						</table>
						
					</div>
				</div>
			</div>
			<!--    ///////////////////      copyEmailDetails VIEW END    ///////////////////            -->
				...
				<div class="viewerContainer">
					...
					<div class="viewerBody">
						...
						api="<?=api_root; ?>requisitions/rfqs/add_new_requisition_rfq.php">
							...
							<div id="selectedrfqItems" style="display:none;" >
								<table id="supplierSelector" class="tabler" border="1">
									...
									<tbody id="rfq_supps_approved"></tbody>
								</table></div>
								<div id="rfqSups" style="display:none;" ></div>
								
								<div class="viewerBodyButtons">
									<button type="button" onclick="createRFQs();emailRFQ();"><?=lang("Create_RFQs", "AAR"); ?></button>
