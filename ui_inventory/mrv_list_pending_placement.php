<?php
	...
	$showPerPage = 20;
	$totPages = 0;
	$qu_pur_requisitions_sel = "SELECT count(*) FROM  `inv_mrvs` WHERE `mrv_status` = 'pending_placement'";
					...
					<script>
						...
						function bindData( response ){
							...
							for( i=0 ; i < response.length ; i ++ ){
								cc++;
								var button = "";
								if( response[i].mrv_status== 'pending_placement' ){
									button = '<a href="mrv_place_items_01.php?mrv_id='+response[i].mrv_id+'" class=""><button type="button"><?=lang("Place_items", "AAR"); ?></button></a>';
								}
								...
								'	<div class = "td" onclick="showPoDetails('+response[i].mrv_id+ ", '" +response[i].mrv_ref+ "'" +  ",'viewPOdetails'" + ');"><span id="poREF-'+response[i].mrv_id+'" class="text-primary">'+response[i].mrv_ref+'</span></div>'+
