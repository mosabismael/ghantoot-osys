<?php
	...
	$showPerPage = 20;
	$totPages = 0;
	$qu_pur_requisitions_sel = "SELECT count(*) FROM  `inv_mrvs` WHERE `mrv_status` = 'inspection_required' ";
					...
					<script>
						...
						function bindData( response ){
							...
							for( i=0 ; i < response.length ; i ++ ){
								cc++;
								var button = "";
								if( response[i].mrv_status == 'inspection_required' && response[i].inspected_byID == response[i].employee_id ){
									button = '<a href="mrv_approval.php?mrv_id='+response[i].mrv_id+'" class=""><button type="button"><?=lang("Start_Approvals", "AAR"); ?></button></a>';
