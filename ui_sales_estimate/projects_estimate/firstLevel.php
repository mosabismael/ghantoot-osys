...
<script>
	...
	function getLvl1Data(response){
		...
		for( i=0 ; i < response.length ; i ++ ){
			...
			'<div class = "td"><button   '+onclick+'>'+response[i].level1_name+'</button> </div>'+
	...
	function getLvl2Data(response, id, name){
		$('#level2Body').html('');
		
		var cc = 0;
		if( name == 'Tools and Equipment'){
			$('#other-qty').show();
			$('#other-cost').show();
			$('#other-prodType').hide();
			$('#other-startEst').hide();
		...
		else if(name == 'Others' || name == 'Services and Sub Contractor'){
			$('#other-qty').show();
			$('#other-cost').show();
			$('#other-prodType').hide();
			$('#other-startEst').hide();
		...
		for( i=0 ; i < response.length ; i ++ ){
			var extraCol = '';
			var editCol = '';
			if(name == 'Others' || name == 'Services and Sub Contractor' || name == 'Tools and Equipment'){
				var dur = '';
				if(name == 'Tools and Equipment'){
					dur = '<div class = "td">'+response[i].duration+'</div>';
				}
				extraCol = '<div class = "td">'+response[i].quantity+'</div>'+
				dur+
				'<div class = "td">'+response[i].cost+'</div>';
				
				editCol = '<div class = "td"><i class="far fa-edit " onclick = "openEdit('+ "'" +response[i].level2_name+ "'" +","+response[i].level2_id+","+ "'" +response[i].quantity+ "'" +","+response[i].cost+  ", 'addProductLevel2'"+')"></i></div>';
				...
				extraCol = '<div class = "td">'+response[i].type_name+'</div>'+
				'<div class = "td" id = "completed-2-'+response[i].level2_id+'" style = "width: 10%;">'+boq_td+
				'</div>';
				
				editCol = '<div class = "td"><i class="far fa-edit " onclick = "openEdit('+ "'" +response[i].level2_name+ "'" +","+response[i].level2_id+","+ "'" +response[i].level2_description+ "'" +","+response[i].type_id+  ", 'addProductLevel2'"+')"></i></div>';
			...
			// '<div class = "td" ><input  type="submit"  value='+response[i].level2_name+' '+onclick+'>'+'</input> </div>'+
			'<div class = "td" ><button '+onclick+'>'+response[i].level2_name+'</button> </div>'+
