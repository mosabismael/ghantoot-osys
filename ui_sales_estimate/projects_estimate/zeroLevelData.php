<?php
	...
	if(mysqli_num_rows($qu_project_level1_EXE)){
		while($project_level1_REC = mysqli_fetch_assoc($qu_project_level1_EXE)){
			...
			$qu_prject_sel = "SELECT count(*) as count , sum(quantity *cost*duration) as cost FROM `z_project_level2` WHERE `level1_id` = $level1_id "  ;
			...
			if(mysqli_num_rows($qu_project_EXE)){
				...
				$total_amount = $qu_project_REC['cost'];
				if($count != 0){
