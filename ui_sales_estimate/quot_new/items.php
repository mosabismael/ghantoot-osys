...
<?php
	...
	if(isset($pID) && $pID != ''){
			$count = 0;
			$qu_select = "select count(*) as count from sales_quotations_items where quotation_id = '$pID'";
...
<script>
	...
	function add_item(){
		...
		if( item_unit != 0 ){
			...
			if(item_name != ""){
				...
				'' + inputer +
				'</tr>';
				add_quot_item(item_name , item_price , item_qty , item_unit );
	...
	function add_quot_item(item_name , item_price , item_qty , item_unit){
		
		$.ajax({
			url      :"<?=api_root; ?>sales/quotations/add_quot_item.php",
			data     :{'item_name': item_name, 'item_price': item_price, 'item_qty':item_qty , 'item_unit':item_unit, 'rfq_id':<?=$pID?>},
			dataType :"JSON",
			type     :'POST',
