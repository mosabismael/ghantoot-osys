<?php
	...
	$sNo = $start + 1;
	$qu_purchase_orders_sel = "SELECT * FROM  `purchase_orders` WHERE ((`po_status` <> 'draft') $serchCond) ORDER BY `po_id` ASC LIMIT $start,$per_page";
	...
	if(mysqli_num_rows($qu_purchase_orders_EXE)){
		while($purchase_orders_REC = mysqli_fetch_assoc($qu_purchase_orders_EXE)){
			...
			$requisition_id = ( int ) $purchase_orders_REC['requisition_id'];
